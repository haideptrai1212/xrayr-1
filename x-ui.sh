...
if [ $(getconf WORD_BIT) != '32' ] && [ $(getconf LONG_BIT) != '64' ]; then
    echo "X-ui không hỗ trợ hệ thống 32-bit(x86), vui lòng sử dụng hệ điều hành 64 bit(x86_64)!"
    exit -1
fi
...
config_after_install() {
    echo -e "${yellow}Cài đặt/cập nhật đã hoàn tất! Để bảo mật, nên sửa đổi cài đặt bảng điều khiển ${plain}"
    read -p "Bạn có muốn tiếp tục sửa đổi [y/n] không? ": config_confirm
    if [[ "${config_confirm}" == "y" || "${config_confirm}" == "Y" ]]; then
        read -p "Username:" config_account
        echo -e "${yellow}Your username: ${config_account}${plain}"
        read -p "Password: " config_password
        echo -e "${yellow}Your password: ${config_password}${plain}"
        read -p "Please set up the panel port:" config_port
        echo -e "${yellow}Cổng bảng điều khiển của bạn là: ${config_port}${plain}"
        echo -e "${yellow}Đang khởi chạy, vui lòng đợi...${plain}"
        /usr/local/x-ui/x-ui setting -username ${config_account} -password ${config_password}
        echo -e "${yellow}Tên tài khoản và mật khẩu được đặt thành công!${plain}"
        /usr/local/x-ui/x-ui setting -port ${config_port}
        echo -e "${yellow}Đã thiết lập cổng bảng điều khiển thành công!${plain}"
    else
        ...
        if [[ ! -f "/etc/x-ui/x-ui.db" ]]; then
            ...
            echo -e "Đây là bản cài đặt mới, sẽ tạo thông tin đăng nhập ngẫu nhiên vì lo ngại về bảo mật:"
            echo -e "###############################################"
            echo -e "${green}username:${usernameTemp}${plain}"
            echo -e "${green}password:${passwordTemp}${plain}"
            echo -e "###############################################"
            echo -e "${red}Nếu bạn quên thông tin đăng nhập, bạn có thể gõ x-ui rồi gõ 7 để kiểm tra sau khi cài đặt${plain}"
        else
            echo -e "${red} Đây là bản nâng cấp của bạn, sẽ giữ nguyên cài đặt cũ, nếu bạn quên thông tin đăng nhập, bạn có thể nhập x-ui rồi gõ 7 để kiểm tra${plain}"
        fi
    fi
